-- ============================================================
-- sp_tx_prime_batch
-- Processes an array of transactions in a single call
-- Much faster than calling sp_tx_prime in a loop
--
-- Usage:
--   CALL sp_tx_prime_batch('[{...}, {...}, {...}]', @count);
--   SELECT @count;  -- Returns number processed
--
-- Input: JSON array of transaction objects from Solscan API
-- ============================================================
...
BEGIN
    ...
    WHILE v_idx < v_tx_count DO
        ...
        IF v_signature IS NOT NULL AND v_signature != 'null' THEN
            ...
            -- Build program name lookup from parsed_instructions
