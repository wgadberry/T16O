-- tx_guide table
-- Graph edge table for fund flow traversal (theGuide)
-- One row per value movement: from_address -> to_address
...
CREATE TABLE IF NOT EXISTS `tx_guide` (
  ...
  -- Edge endpoints - OWNERS (wallets - primary for graph traversal)
  ...
  -- Edge endpoints - TOKEN ACCOUNTS (ATAs - secondary for audit trail)
  ...
  -- Provenance (trace back to source record)
