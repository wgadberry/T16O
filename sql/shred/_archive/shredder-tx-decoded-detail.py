...
async def fetch_endpoint(session: aiohttp.ClientSession, endpoint: str, signatures: List[str], max_retries: int = 3) -> dict:
    """Fetch data from a Solscan endpoint asynchronously with retry for transient errors"""
...
def create_api_session() -> aiohttp.ClientSession:
    ...
    timeout = aiohttp.ClientTimeout(total=60)  # Increased from 30s for batch requests
...
class RabbitMQManager:
    """Manages RabbitMQ connection with auto-reconnect"""
    ...
    def publish(self, combined: dict) -> bool:
        """Publish with auto-reconnect on failure"""
...
def setup_rabbitmq(host: str, port: int, user: str, password: str) -> Tuple[Optional[RabbitMQManager], bool]:
    """Setup RabbitMQ manager"""
    manager = RabbitMQManager(host, port, user, password)
    if manager.connect():
        return manager, True
    return None, False
...
async def process_batch_async(
    ...
    try:
        ...
        if use_rabbitmq and rabbitmq_channel:
            # rabbitmq_channel is now a RabbitMQManager with auto-reconnect
            if rabbitmq_channel.publish(combined):
...
async def main_async(args):
    ...
    # Setup RabbitMQ (optional) - now uses RabbitMQManager with auto-reconnect
    ...
    if use_rabbitmq:
        ...
        rabbitmq_manager, connected = setup_rabbitmq(
