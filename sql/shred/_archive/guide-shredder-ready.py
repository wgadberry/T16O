#!/usr/bin/env python3
"""
Guide Shredder Ready - Process 'ready' transactions from tx.enriched queue
Consumes combined decoded+detail JSON from RabbitMQ and shreds into MySQL tables.
Updates tx_state from 'ready' to 'shredded'.

Pipeline position:
    primed → shredder-tx-decoded-detail.py → ready + tx.enriched queue
                                                  ↓
                                        guide-shredder-ready.py (this script)
                                                  ↓
                                             shredded
...
4. Update tx_state to 'shredded'
5. Acknowledge message on success, requeue on failure

Usage:
    python guide-shredder-ready.py [--prefetch 10]
    python guide-shredder-ready.py --max-messages 100
"""
