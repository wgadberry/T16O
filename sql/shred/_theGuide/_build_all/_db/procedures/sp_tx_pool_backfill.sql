...
BEGIN
    ...
    -- Short-circuit: only proceed if there are missing pools
    IF EXISTS (
    ...
    -- Short-circuit: only proceed if there are pools needing backfill with parseable labels
    IF EXISTS (
        ...
        WHERE (p.token1_id IS NULL OR p.token2_id IS NULL OR p.pool_label IS NULL)
          AND a.label IS NOT NULL AND a.label LIKE '%(%-%)%'
        LIMIT 1
    ) THEN
    ...
    -- Short-circuit: only proceed if there are target pools needing account data
    IF EXISTS (
        SELECT 1 FROM tx_pool
        WHERE token_account1_id IS NULL
          AND pool_label IS NOT NULL
          AND pool_label NOT LIKE '%Market%'
        LIMIT 1
    ) THEN
