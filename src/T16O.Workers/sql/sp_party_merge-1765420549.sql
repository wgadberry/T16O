...
-- Added 21 new action type conditions
-- New ENUM values required: deposit, sync, withdraw
-- ============================================================
--
...
-- ============================================================
-- STEP 1: Add new ENUM values to party.action_type
-- ============================================================
-- Run this ALTER TABLE before running the stored procedure update
--

ALTER TABLE party MODIFY COLUMN action_type ENUM(
    'airdrop',
    'burn',
    'closeAccount',
    'createAccount',
...
proc_body: BEGIN
    ...
    SET tb.action_type = CASE
        ...
        -- ============================================================
        -- Winston Worker Additions (2025-12-11 02:35:49 UTC)
        -- ============================================================

        -- Deposit instruction (172 occurrences)
        WHEN v_log_messages LIKE '%Instruction: Deposit%'
        THEN 'deposit'

        -- Mip1Sell instruction (106 occurrences)
        WHEN v_log_messages LIKE '%Instruction: Mip1Sell%'
        THEN 'swap'

        -- CreateTokenAccount instruction (82 occurrences)
        WHEN v_log_messages LIKE '%Instruction: CreateTokenAccount%'
        THEN 'createAccount'

        -- Swap2 instruction (27 occurrences)
        WHEN v_log_messages LIKE '%Instruction: Swap2%'
        THEN 'swap'

        -- Mip1CancelSell instruction (25 occurrences)
        WHEN v_log_messages LIKE '%Instruction: Mip1CancelSell%'
        THEN 'swap'

        -- SwapBaseInput instruction (19 occurrences)
        WHEN v_log_messages LIKE '%Instruction: SwapBaseInput%'
        THEN 'swap'

        -- BuyNft instruction (17 occurrences)
        WHEN v_log_messages LIKE '%Instruction: BuyNft%'
        THEN 'swap'

        -- BuySingleListing instruction (15 occurrences)
        WHEN v_log_messages LIKE '%Instruction: BuySingleListing%'
        THEN 'swap'

        -- TransferFee instruction (8 occurrences)
        WHEN v_log_messages LIKE '%Instruction: TransferFee%'
        THEN 'transfer'

        -- WithdrawWithheldTokensFromMint instruction (4 occurrences)
        WHEN v_log_messages LIKE '%Instruction: WithdrawWithheldTokensFromMint%'
        THEN 'withdraw'

        -- PoolStake instruction (4 occurrences)
        WHEN v_log_messages LIKE '%Instruction: PoolStake%'
        THEN 'stake'

        -- RouteSwap instruction (2 occurrences)
        WHEN v_log_messages LIKE '%Instruction: RouteSwap%'
        THEN 'swap'

        -- SwapV2 instruction (2 occurrences)
        WHEN v_log_messages LIKE '%Instruction: SwapV2%'
        THEN 'swap'

        -- SwapBaseOutput instruction (2 occurrences)
        WHEN v_log_messages LIKE '%Instruction: SwapBaseOutput%'
        THEN 'swap'

        -- SyncNative instruction (1 occurrences)
        WHEN v_log_messages LIKE '%Instruction: SyncNative%'
        THEN 'sync'
        ...
        -- InitializeToken instruction (1 occurrences)
        WHEN v_log_messages LIKE '%Instruction: InitializeToken%'
        THEN 'createAccount'

        -- TransferToSponsor instruction (1 occurrences)
        WHEN v_log_messages LIKE '%Instruction: TransferToSponsor%'
        THEN 'transfer'

        -- VestingClaim instruction (1 occurrences)
        WHEN v_log_messages LIKE '%Instruction: VestingClaim%'
        THEN 'reward'

        -- WithdrawWithheldTokensFromAccounts instruction (1 occurrences)
        WHEN v_log_messages LIKE '%Instruction: WithdrawWithheldTokensFromAccounts%'
        THEN 'withdraw'

        -- ClaimWithMerkle instruction (1 occurrences)
        WHEN v_log_messages LIKE '%Instruction: ClaimWithMerkle%'
        THEN 'reward'
